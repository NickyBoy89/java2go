...
func ParseInterface(sourceString string) ParsedInterface {
  ...
  for ; ci < len(classBody); ci++ {
    ...
    if char == '@' { // Detected an annotation
      newlineIndex := parsetools.FindNextIndexOfChar(classBody[ci:], '\n')
      ...
      closingBrace := parsetools.IndexOfMatchingBrace(classBody, ci)
      ...
      methodWords := parsetools.DiscardBlankStrings(strings.Split(classBody[lastInterest:ci], " "))
      ...
      case "static":
        ...
        ci = closingBracket + 1
      default:
        closingSemicolon := parsetools.FindNextSemicolonIndex(classBody[ci:]) + ci
